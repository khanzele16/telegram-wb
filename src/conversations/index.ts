...
const buy = async (
  ...
  const hasActive = await isPromocode(ctx);
  if (hasActive === false) {
    await ctx.reply(
      `🛒 <b>Приобрести подписку</b>\n\nСтоимость — <b>1990₽</b> за 30 дней.\n\nПосле оплаты вы в течении 10 минут получите доступ в <b>закрытый Telegram-канал</b> 🔒`,
      {
        reply_markup: new InlineKeyboard()
          .url(`Оплатить 1990₽`, await prodamusHref(ctx.from?.id ?? 1))
          .row()
